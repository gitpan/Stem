...
ERR
		...
		return Stem::Portal::send_msg( $self, $to_hub ) ;
	}
...
# not a local cell or named hub, send it to DEFAULT portal
	...
	my $err = Stem::Portal::send_msg( $self ) ;
	return unless $err ;
...
LABEL
	...
	foreach my $type ( @addr_types ) {
		...
		my $addr_text = address_string( $addr ) || 'NONE' ;
